# This test is failing in devtools::check for some reason
#test_that("authentication works", {

#  expect_snapshot(
#    authenticate(email = httr2::secret_decrypt("PF4hPUkS3vBg88tAHPw4usSuACHd_pg92sBfKpJomc0KiwGpz32uap0BEb0",
#                                               "FANTASY_KEY"),
#                 password = httr2::secret_decrypt("SjNB1d71cqrLJq6anSDPf_xWOrTzKH0RZXr20w",
#                                                  "FANTASY_KEY")),
#    transform = function(x) gsub(pattern = "[[:alnum:][:punct:]]+@[[:alnum:][:punct:]]+",
#                                 replacement = "[login]",
#                                 x)
#  )

#})
